...
mongoose.connection.on("connected", () => {
  ...
  // Schedule Price Alert Worker (e.g., every 30 minutes)
    ...
    for (const r of results.slice(0, 3)) {
      await Product.updateOne(
        { url: r.productUrl },
        {
          ...
          ...(r.price != null
            ? { $push: { priceHistory: { price: r.price, at: now } } }
            : {}),
        },
        { upsert: true }
      );
...
app.get("/api/compare", async (req, res) => {
  ...
  const combined = [...amazon, ...flipkart, ...croma, ...reliance].sort(
    (a, b) => {
...
app.post("/api/alerts", auth, async (req, res) => {
  try {
    ...
    if (!product_id || !target_price) {
      return res
        .status(400)
        .json({ error: "user_id, product_id, and target_price are required" });
    }
...
app.get("/api/products/:id/price-history", async (req, res) => {
  try {
    const product = await Product.findById(
      req.params.id,
      "priceHistory title url currentPrice"
    );
    if (!product) {
      return res.status(404).json({ error: "Product not found" });
    }
...
app.get("/api/products/:id/suggested-price", async (req, res) => {
  try {
    ...
    if (currentPrice > avgPrice * 1.1) {
      ...
      reasoning = `Current price (${currentPrice}) is above historical average (${avgPrice.toFixed(
        2
      )}). Suggested price is near average.`;
      confidence = "high";
      ...
      reasoning = `Current price (${currentPrice}) is within normal range. Historical average: ${avgPrice.toFixed(
        2
